class Admin::TokensController < AdminController
  ...
  # GET /tokens.json
  def index
    @tokens = Token.includes(:company).order(:name)
  end

  # GET /tokens/1
  # GET /tokens/1.json
  def show
   @currencies = @token.currencies.select('currencies.*, exchanges.name exchange_name, currency_exchanges.profile currency_exchange_profile').joins(exchanges: :currency_exchanges).uniq
  end
